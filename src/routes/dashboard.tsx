...
import {
  GitBranch, Clock, CheckCircle2, Megaphone, ArrowRight, Sparkles, RotateCcw, Trash2,
  Building2, Lightbulb,
} from "lucide-react";
import {
...
function Dashboard() {
  ...
  const stats = [
    ...
    { label: "Clientes ativos", value: activeClients, icon: Building2, color: "text-fuchsia-400 bg-fuchsia-500/15" },
    { label: "Ideias salvas", value: 6, icon: Lightbulb, color: "text-pink-400 bg-pink-500/15" },
  ];
    ...
    <AppShell>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Visão geral da operação da agência</p>
        </div>
    ...
    <section>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {items.slice(0, 6).map((it) => <GenerationCard key={it.id} item={it} />)}
        </div>
      )}
    </section>
  );
...
function GenerationCard({ item }: { item: GenerationHistoryItem }) {
  return (
    <Card className="p-4 bg-card/70 border-border/60 flex flex-col gap-3">
      ...
      <div>
        <div className="text-xs uppercase tracking-wide text-muted-foreground mb-0.5">{item.client || "Sem cliente"} · Tema</div>
        <div className="text-sm font-medium line-clamp-1">{item.theme || "—"}</div>
      </div>
      {item.hook && (
        <div className="text-sm text-muted-foreground line-clamp-2 leading-relaxed">"{item.hook}"</div>
      )}
      <div className="flex items-center justify-between gap-2 pt-1">
        <div className="text-[11px] text-muted-foreground capitalize">{item.format} · {item.objective}</div>
        <div className="flex gap-1.5">
          <Button size="sm" variant="ghost" className="text-muted-foreground hover:text-destructive" onClick={() => removeGeneration(item.id)} aria-label="Remover">
            <Trash2 className="size-3.5" />
          </Button>
          <Button size="sm" className="bg-gradient-primary shadow-glow hover:opacity-90" onClick={() => openNewPostWithPrefill(item)}>
